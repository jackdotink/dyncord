...
use crate::interactions::message::{MessageCommand, MessageCommandGroup};
        ...
        && let InteractionData::ApplicationCommand(data) = data
    {
        for node in &*event_ctx.handle.interactions {
            let command = match node {
                InteractionNode::MessageCommand(command) => {
    ...
    State: StateBound,
{
    for node in &group.children {
        let command = match node {
            InteractionNode::MessageCommand(subcommand) => match_command(ctx, parts, subcommand),
            InteractionNode::MessageCommandGroup(subgroup) => match_group(ctx, parts, subgroup),
            _ => None,
        };
